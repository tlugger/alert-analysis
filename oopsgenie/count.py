...
class Counter:
    ...
    def format_output(cls, count_map, column, outfile, limit, fuzzy=False):
        ...
        alert_list = sorted(
            count_map.items(), key=lambda kv: (kv[1], kv[0]), reverse=True
        )
...
class FuzzyCounter(Counter):
    @classmethod
    def count(
        cls, file, column, limit, threshold, remove_numbers, outfile, alias_strip_list
    ):
        strip_list = []
        if alias_strip_list:
            ...
            if key not in skip_list:
                for alert_key in alert_keys:
                    if remove_numbers:
                        ...
                        new_alert_key = "".join(
                            [i for i in alert_key if not i.isdigit()]
                        )
                    else:
                        new_key = key
